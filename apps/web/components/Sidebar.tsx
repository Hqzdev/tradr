...
export default function Sidebar() {
  ...
  return (
    ...
    >
      ...
      <div className={clsx("rounded-2xl bg-[#F7F5F8] p-2", settingsOpen ? "animate-profile-drop" : "animate-profile-lift")}>
        <div className={clsx("flex h-[30px] items-center gap-2.5", collapsed && "justify-center")}>
        ...
        {!collapsed && (
          <div className="flex min-w-0 flex-col gap-0.5">
            <p className="truncate text-[12px] text-ink">{user?.displayName ?? "Аккаунт"}</p>
            <p className="truncate text-[10px] text-[#817A87]">{user?.email ?? ""}</p>
          </div>
        )}
        </div>
        {!collapsed && <button type="button" onClick={handleLogout} className="mt-2 w-full text-left text-[11px] text-[#817A87] transition-colors hover:text-negative">Выйти</button>}
      </div>
    </aside>
  );
}
