...
export default function CreateAgentScreen() {
  ...
  const backtest = useMemo(
    () => ({
      returnPercent: strategy === "aggressive" ? 14.2 : strategy === "careful" ? 6.1 : -1.8,
      drawdownPercent: strategy === "aggressive" ? 5.4 : strategy === "careful" ? 1.6 : 8.2,
      winRate: strategy === "aggressive" ? 61 : strategy === "careful" ? 74 : 44,
    }),
    [strategy]
  );
      ...
      <div className="mt-6 grid grid-cols-1 gap-6 lg:grid-cols-[1fr_410px]">
        <Card className="p-6">
          ...
          {step === 3 && (
            <div>
              ...
              <p className="mt-1 text-body-sm text-steel">
                Агент начнёт торговать сразу после создания — на демо-счёте, реальные деньги не
                используются.
              </p>
        ...
        <Card className="p-6">
          ...
          <div className="mt-5 border-t border-bone pt-5">
            <p className="text-body-sm font-[485] text-ink">Бэктест · 90 дней демо-данных</p>
            <div className="mt-3 grid grid-cols-3 gap-3 text-center">
              <div>
                <p
                  className={clsx(
                    "text-body font-[485] tabular-nums",
                    backtest.returnPercent >= 0 ? "text-positive" : "text-negative"
                  )}
                >
                  {backtest.returnPercent >= 0 ? "+" : "−"}
                  {Math.abs(backtest.returnPercent).toFixed(1)}%
                </p>
                <p className="mt-0.5 text-caption text-steel">Доходность</p>
              </div>
              <div>
                <p className="text-body font-[485] tabular-nums text-negative">
                  −{backtest.drawdownPercent.toFixed(1)}%
                </p>
                <p className="mt-0.5 text-caption text-steel">Просадка</p>
              </div>
              <div>
                <p className="text-body font-[485] tabular-nums text-ink">{backtest.winRate}%</p>
                <p className="mt-0.5 text-caption text-steel">Win rate</p>
              </div>
            </div>
          </div>
        </Card>
